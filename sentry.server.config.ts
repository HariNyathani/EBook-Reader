// Sentry server configuration — Phase 15 (ISD §15.G, §15.V).
// Imported by @sentry/nextjs when the server SDK initializes.
    ...
    beforeSend(event) {
      ...
      if (event.request) {
        ...
        if (event.request.headers) {
          // Redact every auth/cookie header server-side.
