 ...
 *   - `style-src 'self' 'nonce-…'` — we nonce Tailwind/runtime
 *     styles. (Foliate itself injects styles INTO its own iframe, so
 *     it doesn't need any policy on the parent page; the iframe is
 *     sandboxed.)
 *
 ...
 *   - style-src 'self' 'nonce-…'    — strict
 *   - img-src 'self' blob: data:    — for foliate covers/thumbs
 *   - font-src 'self' data:         — for reader fonts
 *   - connect-src 'self' [supabase] [sentry] [app]
...
export function buildCsp(ctx: CspBuildContext): string {
  ...
  // 3. style-src — strict. We nonce inline styles injected by the
  // framework and any of our own inline <style> tags. 'unsafe-inline'
  // is NOT used.
  directives.push(`style-src 'self' 'nonce-${ctx.nonce}'`);
